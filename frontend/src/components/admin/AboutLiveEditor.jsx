...
export default function AboutLiveEditor({ isAdmin, onSaveOverride, onUploadImage }) {
  ...
  useEffect(() => {
    ...
    editableNodes.forEach((node) => {
      if (canOpen) {
        node.classList.add("about-edit-highlight");
      } else {
        node.classList.remove("about-edit-highlight");
      }
    });

    return () => {
      editableNodes.forEach((node) => node.classList.remove("about-edit-highlight"));
    };
  }, [canOpen]);
  ...
  const panelMessage = useMemo(() => {
    ...
    return "Click highlighted text or image";
    ...
    <>
      <div className="fixed bottom-5 right-5 z-[1000] w-[280px] rounded-2xl border border-[#d9c8b3] bg-white/95 p-4 shadow-2xl backdrop-blur">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[#3f2d1f]">About Editor</h3>
          <button
            type="button"
            onClick={() => {
