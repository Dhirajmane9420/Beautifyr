...
export function toProductPayload(input) {
  ...
  return {
    ...
    image: images[0] || "https://via.placeholder.com/700x700",
    images: images.length ? images : ["https://via.placeholder.com/700x700"],
    price: resolvedPrice,
    originalPrice: resolvedOriginalPrice,
    size: liquid ? normalizeMlSize(fallbackSize) : "",
    sizes: liquid ? sizeOptions.map((size) => normalizeMlSize(size)) : [],
    sizeStock,
    sizeVariants,
    selectedSizeVariant,
    inStock:
      typeof input?.inStock === "boolean"
        ? input.inStock
        : sizeVariants.length
          ? sizeVariants.some((entry) => entry.stock > 0)
          : sizeStock.length
            ? sizeStock.some((entry) => entry.stock > 0)
          : true,
    isLiquid: liquid,
  };
}
